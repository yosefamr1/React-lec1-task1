...
function ProductCard({ product }) {
    ...
    const renderStars = (rating) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (rating >= i) {
                stars.push(<span key={i} style={{ color: '#f5c518', fontSize: '20px' }}>★</span>);
            } else if (rating >= i - 0.5) {
                stars.push(
                    <span key={i} style={{ position: 'relative', display: 'inline-block', fontSize: '20px' }}>
                        <span style={{ color: '#f5c518', position: 'absolute', width: '50%', overflow: 'hidden' }}>★</span>
